(() => {
  ...
  function timerForLevel(level) { return (level === 1 ? 72 : lerp(62, 24, phase01(level))) + 20; }
  ...
  function updateSoundButton() {
    ...
    soundButton.textContent = on ? 'SOUND ON' : 'ENABLE SOUND';
  ...
  function sliderSound(value) {
    ...
    simpleTone(base, 0.11, 'square', 0.34, 150);
    simpleTone(base * 1.98, 0.09, 'square', 0.22, -80, 0.012);
    if (n > 0.28) noiseBurst(0.06, 0.03 + n * 0.03, 950);
  }
  ...
  function buildAlignmentLevel(level, base) {
    ...
    return {
      ...
      tolerance: isStarter ? 0.42 : clamp(1.02 - level * 0.0067, 0.16, 1.02),
      depthTolerance: isStarter ? 0.38 : clamp(1.06 - level * 0.0063, 0.22, 1.06),
      magnetRadius: isStarter ? 0.16 : lerp(0.48, 0.05, phase01(level)),
  ...
  function buildMaskShape(level) {
    const pts = [];
    const edges = [];
    const addPt = (x, y, z = 0) => (pts.push({ x, y, z }), pts.length - 1);
    ...
    const surfaceZ = (x, y, extra = 0) => {
      const xn = x / 155;
      const yn = y / 185;
      const dome = 16 * Math.max(0, 1 - (xn * xn * 0.92 + yn * yn * 0.78));
      const cheekLift = 3.8 * Math.max(0, 1 - Math.abs(x) / 150) * Math.max(0, 1 - Math.abs((y - 18) / 180));
      return dome + cheekLift + extra;
    };

    const arc = (cx, cy, rx, ry, steps, start = Math.PI, end = 0, extraFn = null) => {
      const arr = [];
      for (let i = 0; i < steps; i++) {
        const t = start + (end - start) * (i / Math.max(1, steps - 1));
        const x = cx + Math.cos(t) * rx;
        const y = cy + Math.sin(t) * ry;
        const extra = extraFn ? extraFn(x, y, t, i / Math.max(1, steps - 1)) : 0;
        arr.push(addPt(x, y, surfaceZ(x, y, extra)));
      }
      return arr;
    };

    const oval = (cx, cy, rx, ry, steps, wobble = 0, extraFn = null) => {
      const arr = [];
      for (let i = 0; i < steps; i++) {
        const t = i / steps * Math.PI * 2;
        const x = cx + Math.cos(t) * rx;
        const y = cy + Math.sin(t) * ry + Math.sin(t * 2) * wobble;
        const extra = extraFn ? extraFn(x, y, t, i / steps) : 0;
        arr.push(addPt(x, y, surfaceZ(x, y, extra)));
      }
      return arr;
    };

    // Flatter theatrical mask surface: one curved plane, no deep nose ridge.
    const topMid = addPt(0, -178, surfaceZ(0, -178, 0));
    const foreheadMid = addPt(0, -124, surfaceZ(0, -124, 4));
    const bridge = addPt(0, -58, surfaceZ(0, -58, 5));
    const noseMid = addPt(0, -4, surfaceZ(0, -4, 4));
    const philtrum = addPt(0, 48, surfaceZ(0, 48, 2));
    const upperLipMid = addPt(0, 88, surfaceZ(0, 88, 1));
    const lowerLipMid = addPt(0, 112, surfaceZ(0, 112, 0));
    const chinTip = addPt(0, 176, surfaceZ(0, 176, -1));
    const centerSpine = [topMid, foreheadMid, bridge, noseMid, philtrum, upperLipMid, lowerLipMid, chinTip];
    chain(centerSpine);

    const leftContour = [
      addPt(-54, -174, surfaceZ(-54, -174, -2)), addPt(-96, -164, surfaceZ(-96, -164, -3)), addPt(-128, -140, surfaceZ(-128, -140, -3)),
      addPt(-144, -90, surfaceZ(-144, -90, -4)), addPt(-147, -22, surfaceZ(-147, -22, -4)), addPt(-136, 44, surfaceZ(-136, 44, -4)),
      addPt(-111, 106, surfaceZ(-111, 106, -4)), addPt(-74, 150, surfaceZ(-74, 150, -3)), addPt(-34, 174, surfaceZ(-34, 174, -2))
    ];
    const rightContour = leftContour.map(i => {
      const p = pts[i];
      return addPt(-p.x, p.y, p.z);
    });
    chain([topMid, ...leftContour, chinTip]);
    chain([topMid, ...rightContour, chinTip]);

    const foreheadOuter = arc(0, -154, 108, 28, 11, Math.PI * 0.98, Math.PI * 0.02, () => -2);
    const foreheadInner = arc(0, -126, 84, 18, 9, Math.PI * 0.98, Math.PI * 0.02, () => 2);
    chain(foreheadOuter); chain(foreheadInner); connectPairs(foreheadOuter, foreheadInner);

    const leftTemple = arc(-114, -56, 28, 92, 8, -Math.PI / 2, Math.PI / 2, () => -3);
    const rightTemple = arc(114, -56, 28, 92, 8, Math.PI / 2, Math.PI * 1.5, () => -3);
    chain(leftTemple); chain(rightTemple);

    const leftBrow = arc(-46, -70, 36, 10, 7, Math.PI * 0.97, Math.PI * 0.08, () => 2);
    const rightBrow = arc(46, -70, 36, 10, 7, Math.PI * 0.92, Math.PI * 0.03, () => 2);
    chain(leftBrow); chain(rightBrow);

    const leftEyeOuter = oval(-46, -30, 28, 14, 14, 0.8, () => -2);
    const rightEyeOuter = oval(46, -30, 28, 14, 14, 0.8, () => -2);
    const leftEyeInner = oval(-46, -30, 16, 8, 10, 0.4, () => -3);
    const rightEyeInner = oval(46, -30, 16, 8, 10, 0.4, () => -3);
    ...
    const noseL = [bridge, addPt(-11, -26, surfaceZ(-11, -26, 3)), addPt(-18, 12, surfaceZ(-18, 12, 1.5)), addPt(-18, 38, surfaceZ(-18, 38, 0.5))];
    const noseR = [bridge, addPt(11, -26, surfaceZ(11, -26, 3)), addPt(18, 12, surfaceZ(18, 12, 1.5)), addPt(18, 38, surfaceZ(18, 38, 0.5))];
    chain(noseL); chain(noseR);
    const noseBridgeCrossL = addPt(-12, -6, surfaceZ(-12, -6, 2));
    const noseBridgeCrossR = addPt(12, -6, surfaceZ(12, -6, 2));
    addEdge(noseBridgeCrossL, noseBridgeCrossR);
    const nostrilArc = arc(0, 22, 26, 12, 9, Math.PI * 0.94, Math.PI * 0.06, () => -1.5);
    const nostrilInner = arc(0, 24, 16, 5.5, 7, Math.PI * 0.9, Math.PI * 0.1, () => -2);
    chain(nostrilArc); chain(nostrilInner);
    connectPairs(nostrilArc.slice(1, 8), nostrilInner);

    const leftCheekUpper = arc(-64, 10, 62, 34, 9, Math.PI * 1.02, Math.PI * 0.1, () => 1);
    const rightCheekUpper = arc(64, 10, 62, 34, 9, Math.PI * 0.9, -Math.PI * 0.02, () => 1);
    const leftCheekLower = arc(-54, 68, 54, 30, 8, Math.PI * 1.02, Math.PI * 0.12, () => 0.5);
    const rightCheekLower = arc(54, 68, 54, 30, 8, Math.PI * 0.88, -Math.PI * 0.02, () => 0.5);
    chain(leftCheekUpper); chain(rightCheekUpper); chain(leftCheekLower); chain(rightCheekLower);

    const upperLip = oval(0, 98, 46, 12, 16, -1.4, (x, y, t) => Math.sin(t) > 0 ? 0.8 : 0.2);
    const lowerLip = oval(0, 116, 40, 15, 16, 1.0, () => -0.4);
    const mouthInner = oval(0, 108, 20, 5.5, 12, 0, () => -2.6);
    ...
    const jawInnerL = arc(-42, 136, 34, 30, 7, Math.PI * 1.0, Math.PI * 0.1, () => -1.5);
    const jawInnerR = arc(42, 136, 34, 30, 7, Math.PI * 0.9, 0, () => -1.5);
    const chinArc = arc(0, 158, 56, 18, 9, Math.PI * 0.98, Math.PI * 0.02, () => -1.5);
    ...
    const scaffolds = [
      ...
      [leftEyeOuter[2], noseL[1]],[leftEyeOuter[11], noseL[2]],[rightEyeOuter[5], noseR[1]],[rightEyeOuter[8], noseR[2]],
      [noseL[3], nostrilArc[2]],[noseR[3], nostrilArc[6]],[nostrilArc[2], philtrum],[nostrilArc[6], philtrum],[philtrum, upperLipMid],
    ...
    const ordered = [];
    const pushRange = (start, end) => { for (let i = start; i < Math.min(end, edges.length); i++) ordered.push(edges[i]); };
    ordered.push(edges[0], edges[1], edges[2]);
    pushRange(8, 26);
    pushRange(3, 8);
    pushRange(26, 46);
    pushRange(46, 62);
    pushRange(62, 76);
    pushRange(76, 103);   // focus growth into eyes/surface, keep end at 103 lines
    if (ordered.length < 103) pushRange(103, 155);
    if (ordered.length < 103) pushRange(155, 189);
    if (ordered.length < 103) pushRange(189, 237);
    if (ordered.length < 103) pushRange(237, 260);
    if (ordered.length < 103) pushRange(260, edges.length);

    const maxLines = Math.min(103, ordered.length);
    const progress = Math.max(0, (level - 1) / 99);
    const lineCount = Math.min(maxLines, 3 + Math.floor(Math.pow(progress, 1.12) * (maxLines - 3)));
